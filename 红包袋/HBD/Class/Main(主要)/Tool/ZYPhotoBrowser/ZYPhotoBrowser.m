...
- (void)setupToolbars
{
    ...
    page.pageIndicatorTintColor = [UIColor colorWithHexString:@"#cdcdcd"];
    page.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#f6ab00"];
    ...
    [_pageControl mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.bounds.size.height - 13 );
    }];
...
- (void)layoutSubviews
{
    ...
    if (_pageControl) {
        [_pageControl mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.bounds.size.width * 0.5 - _pageControl.frame.size.width * 0.5);
            make.top.mas_equalTo(self.bounds.size.height  - 37);
        }];
    }
}
...
- (void)show
{
    ...
    _contentView.backgroundColor = [[UIColor colorWithHexString:@"#0000004c"] colorWithAlphaComponent:0.3];
    ...
    self.center = CGPointMake(_contentView.bounds.size.width * 0.5, _contentView.bounds.size.height * 0.5 + 16);
//    self.bounds = CGRectMake(0, 0, _contentView.bounds.size.width, _contentView.bounds.size.height);
    ...
    CGFloat butWidth = 25.0;
    CGFloat buty = CGRectGetMinY(self.frame) - 32.0 - butWidth;
    closeBut.frame = CGRectMake(SCREEN_WIDTH - 27 - butWidth, buty, butWidth, butWidth);
    [closeBut setImage:[UIImage imageNamed:@"close"] forState:UIControlStateNormal];
    [closeBut addTarget:self action:@selector(closeBut) forControlEvents:UIControlEventTouchUpInside];

    [_contentView addSubview:closeBut];
...
/**
 新添加的需求，原来的有位置，现在不给加位置了。但是原来的代码没变
 */
- (void)closeBut
{
