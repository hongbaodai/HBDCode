...
#define kCircleMargin 50.0              // 圆点离屏幕左边距
#define kCircleDiameter (65.0*BXSize.width)/375.0  // 圆点直径
...
- (void)initCircles
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        int x = kCircleMargin + (i%3) * (kCircleDiameter+(BXSize.width-kCircleMargin*2- kCircleDiameter *3)/2) -20;
        int y = kCircleMargin + (i/3) * (kCircleDiameter+(BXSize.width-kCircleMargin*2- kCircleDiameter *3)/2) -20;
    ...
    for (UIButton *thisbutton in buttonArray) {
        ...
        if (fabs(xdiff) < 36 &&fabs (ydiff) < 36){
            ...
            if (!thisbutton.selected) {
                ...
                if ([self.delegate respondsToSelector:@selector(didUpdateLockString:)]) {
                    [self.delegate didUpdateLockString:string];
                }
