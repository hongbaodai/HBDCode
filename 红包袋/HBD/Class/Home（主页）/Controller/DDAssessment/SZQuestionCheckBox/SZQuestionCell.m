...
//    self.configure.automaticAddLineNumber = NO;
//    NSString *title = self.configure.automaticAddLineNumber ? [NSString stringWithFormat:@"%zd、%@", self.questionNum, dict[@"title"]] : dict[@"title"];
    ...
    numStr.textColor = [UIColor whiteColor];
    numStr.textAlignment = NSTextAlignmentCenter;
    numStr.text = [NSString stringWithFormat:@"%ld",self.questionNum];
    
    // title文字
    UITextView *titleText = [[UITextView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(numStr.frame) + 3, self.configure.topDistance - 2, titleWidth, height + 20)];
    titleText.textColor = self.configure.titleTextColor;
    titleText.font = [UIFont systemFontOfSize:self.configure.titleFont];
    titleText.text = title;
    titleText.editable = NO;
    [self addSubview: numStr];
...
//                lbl.text = [NSString stringWithFormat:@"%@、%@", self.letterArray[i], optionArray[i]];
                //waring:  不拼接字母
                lbl.text = [NSString stringWithFormat:@"%@", optionArray[i]];
                [self addSubview:lbl];
